...
class ScatterData {
  ...
  binBy(metaKey, nbins) {
    const dataSubsets = binBy(this.data, metaKey, nbins)
    const scatterDataSubsets = _.mapObject(dataSubsets, (records, key) => {
      return new ScatterDataSubset(records)
    })
    return scatterDataSubsets
  ...
  binBy2(metaKey, domain) {
    const dataSubsets = binBy2(this.data, metaKey, domain)
    const scatterDataSubsets = _.mapObject(dataSubsets, (records, key) => {
      return new ScatterDataSubset(records)
    })
    return scatterDataSubsets
  ...
  calculateColorScale(metaKey) {
    ...
    if (uniqueCats.indexOf(RARE) !== -1) {
      if (uniqueCats.length === 20) {
        const idx = uniqueCats.indexOf(RARE)
        let greyIdx2 = 15
        if (uniqueCats.indexOf('unknown') !== -1) {
          greyIdx2 = 14
        }
    ...
    const min_score = colorExtent[0]
    const max_score = colorExtent[1]
  ...
  calculateShapeScale(metaKey) {
    const symbols = d3.svg.symbolTypes.map(t => d3.svg.symbol().type(t)())
    ...
    if (meta.type === 'number' && meta.nUnique > 6) {
      ...
      const binnedValues = binValues(_.pluck(this.data, metaKey), 6)
...
class Scatter3dView extends React.Component {
  ...
  shapeBy(metaKey) {
    ...
    const {
      ...
      shapeLabels,
      scatterDataSubsets
    } = model.calculateShapeScale(metaKey)
